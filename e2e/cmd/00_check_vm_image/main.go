...
func action(ctx context.Context, _ *command.Command) error {
	...
	versionParts := strings.Split(latest.Version, ":")
	latestMarketplaceBuild := versionParts[len(versionParts)-1]

	versionParts = strings.Split(latestCustomImageVersion, ":")
	latestCustomBuild := versionParts[len(versionParts)-1]
	...
	// - Y: minor version, replicates version of the Marketplace VM, e.g. 23.10.202310110
	// - Z: patch version, incremented for consecutive builds of the same minor version
